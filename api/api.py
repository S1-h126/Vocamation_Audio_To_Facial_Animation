...
try:
    model = keras.models.load_model(os.path.join(
        app.config['STATIC_SOURCE'], 'BI_LSTM_512_30epochs_dropout03.h5'))
except OSError as e:
    model = None
...
def file_upload():
    ...
    if filename != '':
        ...
        if file_ext not in app.config['UPLOAD_EXTENSIONS']:
            return flask.jsonify(status=404, message='Invalid file extension')
    ...
    try:
        ffmpeg.input(destination).output(
            converted_path, ar=16000, ac=1, format="wav"
        ).run(overwrite_output=True)
        print(f"File converted successfully: {converted_path}")
    except Exception as e:
        return flask.jsonify(status=500, message=f'FFmpeg conversion error: {str(e)}')

    # Use the converted file for ML processing
    if model is None:
        return flask.jsonify(status=404, message='Model not loaded')

    # Pass the converted file to the ML model
    audio_file, loaded_sample_rate = load(converted_path)
